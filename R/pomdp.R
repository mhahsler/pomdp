...
pomdp <- function(discount = 0,
  ...
  ### model
  model <- list(discount = discount, states = states , actions = actions , observations = observations , start = start , transition_prob = transition_prob , observation_prob = observation_prob , reward = reward)
  ...
  if (is.data.frame(transition_prob)) {
    ...
    for (i in 1:dim(transition_prob)[1]) {
      code <- paste(c(code,"T:", as.character(transition_prob[i,1]), ":", as.character(transition_prob[i,2]), ":", as.character(transition_prob[i,3]), transition_prob[i,4],  "\n"), collapse = " ")
    }
  }
  ...
  if (!is.data.frame(transition_prob)) {
    ...
    for (i in 1:number_of_actions) {
      ...
      if (any(transition_prob[[actions[i]]] == "uniform") | any(transition_prob[[actions[i]]] == "identity")) {
  ...
  if (is.data.frame(observation_prob)) {
    ...
    for (i in 1:dim(observation_prob)[1]) {
      code <- paste(c(code,"O:", as.character(observation_prob[i,1]), ":", as.character(observation_prob[i,2]), ":", as.character(observation_prob[i,3]), observation_prob[i,4],  "\n"), collapse = " ")
    }
  }
  ...
  if (!is.data.frame(observation_prob)) {
    ...
    for (i in 1:number_of_actions) {
      ...
      if (any(observation_prob[[actions[i]]] == "uniform") | any(observation_prob[[actions[i]]] == "identity")) {
  ...
  if (is.data.frame(reward)) {
    ...
    for (i in 1:dim(reward)[1]) {
      code <- paste(c(code,"R:", as.character(reward[i,1]), ":", as.character(reward[i,2]), ":", as.character(reward[i,3]), ":", as.character(reward[i,4]), reward[i,5],  "\n"), collapse = " ")
    }
  }
  ...
  ## importing pg file
  pg <- read.table(pg_filename, header = FALSE, sep = " ", quote = "\"", dec = ".", na.strings = NA, numerals = "no.loss")
  pg <- as.matrix(pg)
