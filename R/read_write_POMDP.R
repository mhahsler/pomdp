...
read_POMDP <- function(file, parse_matrices = "sparse") {
  parse_matrices <- match.arg(parse_matrices, c("no", "dense", "sparse"))
...
parse_POMDP_matrix <- function(problem, field = c("T", "O", "R"), actions, states, observations, sparse = TRUE) {
...
.parse_POMDP_reward <- function(problem, actions, states, obs, sparse = TRUE) {
  if (!sparse)
    matrix_list <- lapply(
      actions,
      FUN = function(a) {
  ...
  for (i in ind) {
    ...
    for (action in acts) {
      for (start in starts) {
        ...
        if (length(vals) == 5L) {
          if (end == '*' && obs == '*')
            matrix_list[[action]][[start]][] <- as.numeric(vals[5])
          else if (end == '*')
