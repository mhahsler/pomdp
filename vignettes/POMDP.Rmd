...
* `values` indicates whether the problem is 
a maximization (`values = "reward"`, the default) or a 
minimization (`values = "cost"`), and
...
but it can also be the name of a POMDP file using the format described on 
[www.pomdp.org](http://www.pomdp.org/code/pomdp-file-spec.html).
The `horizon`
