...
The R package **pomdp** provides the infrastructure to define and
...
-   \$T\$\$ a set of conditional transition probabilities
    $T(s' \mid s,a)$ for the state transition $s \rightarrow s'$
    conditioned on the taken action.
...
At each time period, the environment is in some unkown state $s \in S$.
...
-   **`policy`:** A data frame that combines the ingormation from `pg`
